<!doctype html>
<html>
<head>
    <meta charset="utf-8">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css"
          integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">
    <title>Secured HTTP API</title>
    <script type="application/javascript" src="js/keycloak-3.1.0.min.js"></script>
</head>
<body onload="initSSO()">
<div class="container">

    <div>
        <h1>Secure HTTP API - Spring Boot</h1>
        <p class="lead">A miss&atilde;o se expande no Exemplo REST API N&iacute;vel 0, protegendo um endpoint REST usando o
            <a href="https://access.redhat.com/products/red-hat-single-sign-on">Red Hat SSO</a>.
            O Red Hat SSO implementa a especifica&ccedil;&atilde;o OAuth 2.0 e a usa para emitir tokens de acesso para fornecer aos
            clientes v&aacute;rios direitos de acesso a recursos protegidos.
            Proteger um aplicativo com SSO permite adicionar seguran&ccedil;a aos seus aplicativos enquanto centraliza a configura&ccedil;&atilde;o de seguran&ccedil;a.
        </p>

        <h2>Usando o servi&ccedil;o de sauda&ccedil;&atilde;o</h2>
        <p class="lead">O servi&ccedil;o de sauda&ccedil;&atilde;o &eacute; um endpoint protegido.
            Voc&ecirc; precisar&aacute; fazer o login primeiro.
        </p>

        <div class=form-group>
            <button id="login" onclick="keycloak.login()" class="btn">Login</button>
            <button id="logout" onclick="keycloak.logout()" class="btn">Logout</button>
        </div>

        <form class="form-inline">
            <h3>Greeting service (as <span id="user"><i>Unauthenticated</i></span>):</h3>
            <div class=form-group>
                <!-- <form onsubmit="return invokeAPI()"> -->
                <label for="name">Name</label>
                <input type="text" class="form-control" id="name" placeholder="World">
                <label for="group">Group</label>
                <select class="form-control" id="group">
                    <option value="openstack-users">openstack-users</option>
                    <option value="operador">operador</option>
                    <option value="operador-restrito">operador-restrito</option>
                    <option value="any">any</option>
                </select>
                <!-- </form> -->
            </div>
            <button id="invoke" class="btn btn-success">Invoke</button>
        </form>

        <h3>Result:</h3>
        <pre><code id="result">Invoke the service to see the result.</code></pre>

        <h3>Curl command for the command line:</h3>
        <div class=form-group>
            <textarea id="curl" class="form-control" style="font-style: italic;" readonly></textarea>
        </div>
    </div>
</div>

<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"
        integrity="sha384-Tc5IQib027qvyjSMfHjOMaLkfuWVxZxUPnCJA7l2mCWNIpG9mGCD8wGNIcPD7Txa"
        crossorigin="anonymous"></script>

<script>
    // configure the SSO object
    // var keycloak = Keycloak('/jsp/keycloak.jsp');
    var keycloak = Keycloak({
        url: '<%=System.getenv("SSO_AUTH_SERVER_URL") %>/auth',
        realm: '<%=System.getenv("SSO_REALM") %>',
        noCheckCertificate: '<%=System.getenv("SSO_NO_CHECK_CERTIFICATE") == null ? true : false %>',
        disableTrustManager: '<%=System.getenv("SSO_DISABLE_TRUST_MANAGER") == null ? true : false %>',
        clientId: '<%=System.getenv("SSO_RESOURCE") %>',
        enableCors: '<%=System.getenv("SSO_ENABLE_CORS") == null ? true : false %>'
    });

    var bearerToken = null;

    function initSSO() {
        keycloak.init().success(function () {
            // validate that init operation result
            if (keycloak.authenticated) {
                // update UI to show the current user
                $('#user').text(keycloak.tokenParsed.preferred_username);
                bearerToken = keycloak.token;
                // $('#curl').text('curl -H "Authorization: Bearer ' + keycloak.token + '" http://' + window.location.host + '/api/greeting');
            } else {
                bearerToken = null;
                // update UI to show the current user
                $('#user').html('<i>Unauthenticated</i>');
                $('#curl').text('');
            }
        }).error(function (err) {
            alert(err);
        });
    }

    $(document).ready(function () {
        $("#invoke").click(function (e) {
            e.preventDefault();
            //invokeAPI();
            const name = $("#name").val() || "World";
            const role = $("#group").val();

            let nameParam = '';
            if (!(name === null || name === undefined || name === '')) {
                nameParam = '?name=' + encodeURIComponent(name);
            }
            // create a AJAX call
            const url = '//' + window.location.host + '/api/' + role + '/hello' + nameParam;
            $.ajax({
                url: url,
                type: 'GET',
                dataType: 'json',
                beforeSend: function (xhr) {
                    xhr.setRequestHeader('Accept', 'application/json');
                    xhr.setRequestHeader('Authorization', 'Bearer ' + bearerToken);
                }
            }).success(function (res) {
                $('#result').html(JSON.stringify(res));
                $('#result').css('color', 'blue');
            }).error(function (req) {
                alert(`Invocation failed: [${req.status}]`);
                $('#result').html('<b>' + req.status+ '</b>');
                $('#result').css('color', 'red');
            });
            return false;
        });
    });
</script>
</body>
</html>
